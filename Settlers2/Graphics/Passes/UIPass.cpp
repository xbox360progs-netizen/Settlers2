...
UIPass::UIPass(ShaderManager* shaderMgr, GPUTimer* timer)
    : IRenderPass("UIPass", PASS_UI, 3)
...
void UIPass::SetRenderFrame(RenderFrame* frame) {
    m_renderFrame = frame;
}

void UIPass::BeginPass() {
    if (!m_enabled) return;

    if (m_gpuTimer) {
        m_gpuTimerIndex = m_gpuTimer->StartTimer("UIPass");
    }
