...
OverlayFPS::OverlayFPS(IDirect3DDevice9* device, float screenWidth, float screenHeight)
    : m_device(device), m_screenW(screenWidth), m_screenH(screenHeight), m_enabled(false),
      ...
      m_lastTime(0), m_lastGrowTime(0), m_font(nullptr), m_textMgr(nullptr), m_renderQueue(nullptr)
{
...
void OverlayFPS::SetRenderQueue(Graphics::RenderQueue* renderQueue) {
    m_renderQueue = renderQueue;
    if (m_font && m_renderQueue) {
        m_textMgr = new TextManager(m_font, m_screenW, m_screenH, m_renderQueue);
        if (m_font->GetTexture()) {
            m_textMgr->SetFontAtlas(FONT_MENU, m_font->GetTexture());
        }
    }
}
